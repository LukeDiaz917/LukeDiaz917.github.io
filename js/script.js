...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const observe = (targets, options = { threshold: 0.12, rootMargin: '0px 0px -10% 0px' }) => {
  ...
  if (grid) grid.querySelectorAll('.fade-in').forEach((el, i) => el.style.transitionDelay = `${i*100}ms`);
  cards.forEach(el => { if (inViewport(el, 0.15) || prefersReduced) el.classList.add('visible'); });
  observe(cards, { threshold: 0.1, rootMargin: '0px 0px -10% 0px' });

  // Section titles
  const titles = Array.from(document.querySelectorAll('h2'));
  titles.forEach((t,i)=>{ t.classList.add('reveal'); t.style.transitionDelay = `${i*80}ms`; if (inViewport(t,0.15)||prefersReduced) t.classList.add('visible'); });
  ...
  document.querySelectorAll('section').forEach(section => {
    ...
    const perPara = 120;
    paras.forEach((p,i)=>{ p.classList.add('reveal-p'); p.style.transitionDelay = `${baseAfterH2 + i*perPara}ms`; if (inViewport(p,0.1)||prefersReduced) p.classList.add('visible'); });
  ...
  document.querySelectorAll('section ul, section ol').forEach(list => {
    ...
    bullets.forEach((li,i)=>{ li.classList.add('reveal-li'); li.style.transitionDelay = `${i*90}ms`; if (inViewport(li,0.08)||prefersReduced) li.classList.add('visible'); });
  ...
  if (heroImg) {
    if (inViewport(heroImg, 0.05) || prefersReduced) heroImg.classList.add('visible');
...
(() => {
  ...
  const apply = (mode) => {
    if (mode === 'light') { document.documentElement.setAttribute('data-theme','light'); btn.textContent='Theme: Light'; }
    else if (mode === 'dark'){ document.documentElement.setAttribute('data-theme','dark');  btn.textContent='Theme: Dark';  }
    else { document.documentElement.removeAttribute('data-theme'); btn.textContent='Theme: Auto'; }
  };
